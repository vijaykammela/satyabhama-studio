const ALL_CATEGORIES = ["sarees", "lehengas", "kurta-sets", "suits", "indo-western", "jewelry"];
const SORT_OPTIONS = [
 ...
 * `fixedParams` are params baked into the page (e.g. { category: "sarees" }
...
function initFilterablePage(gridEl, countEl, fixedParams = {}) {
  ...
  if (barEl) {
    ...
    const maxPrice = currentParams().maxPrice || "30000";
      ...
      <div class="mb-8 border-b border-ink/10 pb-6">
        ...
        <div id="filter-panel" class="mt-5 hidden flex-col gap-6 sm:flex-row sm:items-start">
          ...
          <div class="min-w-[220px]">
            <p class="mb-3 text-xs uppercase tracking-widest2 text-charcoal/60">Max price: ₹<span id="max-price-label">${Number(maxPrice).toLocaleString("en-IN")}</span></p>
            <input id="price-range" type="range" min="500" max="30000" step="500" value="${maxPrice}" class="w-full" />
          </div>
        </div>
      </div>`;
    ...
    document.getElementById("price-range").addEventListener("input", (e) => {
      document.getElementById("max-price-label").textContent = Number(e.target.value).toLocaleString("en-IN");
    });

...
function initProductPage() {
  ...
  document.title = `${product.name} — Satyabhama Studio`;
      ...
      <div class="grid grid-cols-1 gap-10 lg:grid-cols-2 lg:gap-16">
        <div class="flex flex-col gap-3 sm:flex-row-reverse">
          <div class="relative aspect-[4/5] flex-1 overflow-hidden bg-sand/30">
            ${images[pdpState.activeImage] ? `<img src="${images[pdpState.activeImage]}" alt="${product.name}" class="h-full w-full object-cover" />` : `<div class="flex h-full w-full items-center justify-center text-xs text-charcoal/40">No image</div>`}
          </div>
          ${
            images.length > 1
            ...
            <div class="flex gap-2">
              ${colors
                .map((c) => {
                  ...
                  return `<button onclick="pdpSetColor('${c}')" style="background:${v.colorHex || '#ccc'}" class="h-8 w-8 rounded-full border-2 ${
...
// Products load asynchronously from Supabase — wait for the "products-ready"
// event data-supabase.js dispatches once the fetch completes, instead of
// running on DOMContentLoaded (which would run before PRODUCTS is populated).
document.addEventListener("products-ready", initProductPage);
